...
async function main() {
  ...
  const server=new McpServer({name:'yrkie',version:'0.6.0'});
  ...
  server.registerTool('yrkie_edit_slide',{
    description:'Replace exactly one DOE page with approved complete page Schema, preserving its id and all other pages. Read this page with yrkie_project_slide(includeSchema=true), keep its id and use that response revision. Requires a confirmed outline and ready images. Show the specific change and obtain approval. UUID retries must retain identical input. A revision conflict requires rereading and renewed review; never silently overwrite a concurrent edit.',
